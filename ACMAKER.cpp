...
int solve2(int apos,int wpos, int k) {
	if(apos< 0 && wpos == 0 && k) return 1;
	if(apos <0 || wpos <0) return 0;
	int res = dp[apos][wpos][k];
	if(res!= -1) return res;
	res =0;
	for(int i =k;i<w[wpos].size();i++) 
		if(a[apos] == w[wpos][i]) res += solve2(apos-1,wpos,i+1);
	if(k) res+= solve2(apos,wpos-1,0);
	return res;
}
int main()
{
	...
	while(cin>>n && n)
	{
		...
		while(getline(cin,s) && s!="LAST CASE")
		{
			...
			int res=solve2(a.size()-1,w.size()-1,0);
			if(res==0)
